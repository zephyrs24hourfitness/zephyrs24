  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled || isOpen ? 'bg-brand-black/90 backdrop-blur-lg border-b border-white/5 py-2' : 'bg-transparent py-6'
      }`}
    >
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:block">
            <div className="ml-10 flex items-center space-x-8">
              {navItems.map((item) => (
                item.isExternal ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="font-sans text-[13px] uppercase tracking-[0.15em] font-bold text-white hover:text-brand-red transition-all duration-300 relative group"
                  >
                    {item.label}
                  ...
                  <NavLink
                    ...
                    to={item.path}
                    className={({ isActive }) =>
                      `font-sans text-[13px] uppercase tracking-[0.15em] font-bold transition-all duration-300 relative group ${
              ...
              <NavLink
                to="/plans-and-rates"
                className="bg-brand-red text-white px-6 py-2.5 font-bold uppercase tracking-wider hover:bg-red-700 transition-all duration-300 transform skew-x-[-10deg] shadow-lg hover:shadow-red-900/30"
              >
                <span className="block skew-x-[10deg] text-sm">JOIN NOW</span>
              </NavLink>
            </div>
          </div>
          ...
          <div className="-mr-2 flex lg:hidden">
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-md text-white hover:text-brand-red focus:outline-none transition-colors"
            >
          ...
          {navItems.map((item) => (
            item.isExternal ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block px-3 py-2 text-xl font-display font-bold uppercase tracking-widest w-full text-center text-white hover:text-brand-red transition-colors"
              >
                {item.label}
              </a>
            ) : (
              <NavLink
                ...
                to={item.path}
                className={({ isActive }) =>
                  `block px-3 py-2 text-xl font-display font-bold uppercase tracking-widest w-full text-center hover:text-brand-red transition-colors ${
           ...
           <NavLink
              to="/plans-and-rates"
              className="mt-8 w-full max-w-xs text-center bg-brand-red text-white py-4 font-bold uppercase tracking-widest skew-x-[-6deg]"
            >
